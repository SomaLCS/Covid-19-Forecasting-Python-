# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 14:25:04 2021
...
df_per_State_features["Death Cases"].replace({0:1}, inplace=True)
...
data = df_per_State_features['Death Cases'].astype('double').values
...
train_ml=df_per_State_features.iloc[:int(df_per_State_features.shape[0]*0.7)]
valid_ml=df_per_State_features.iloc[int(df_per_State_features.shape[0]*0.7):]

totActiveCases = activecases.values.reshape(-1,1)
trainActiveCases =totActiveCases[0:int(df_per_State_features.shape[0]*0.7)]
validActiveCases=totActiveCases[int(df_per_State_features.shape[0]*0.7):]

train_dates = df_per_State_features['Date'].iloc[:int(df_per_State_sel_features.shape[0]*0.7)].values
valid_dates = df_per_State_features['Date'].iloc[int(df_per_State_sel_features.shape[0]*0.7):].values
...
train_active =  pd.Series(train_ml['Death Cases'].values, index)
valid_active =  pd.Series(valid_ml['Death Cases'].values, valid_index)
...
plt.title("Death Cases Auto SARIMAX Model Forecasting for Malatsua")
...
plt.title("Daily Death Cases of Covid-19 in Malaysia")
plt.xlabel('Date')
plt.ylabel('Population')
plt.savefig('Covid-19 Death Cases Forecast.png')
plt.show()
...
save(
        collection_id = "Predicted Daily Death Cases",
        document_id = "Daily Death Cases Forecast",
        Data = new_x
    )